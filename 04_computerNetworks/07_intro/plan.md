...
2. Check ARP cache for 192.168.1.20

3. If not found: Broadcast "Who has 192.168.1.20?"

4. Host B responds: "I'm 192.168.1.20, my MAC is AA:BB:CC:DD:EE:FF"

5. Host A caches the mapping

6. Communication proceeds using MAC address
...
4. A checks: "Is 10.0.0.50 on my subnet?" → NO

5. A uses ARP to find THE GATEWAY's MAC address (192.168.1.1)

6. A sends packet to Gateway's MAC, but with C's IP in the packet

7. Gateway receives packet, sees destination IP (10.0.0.50)

8. Gateway uses its routing table to forward

9. Process repeats at each router hop
